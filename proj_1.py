...
#Setting the page title and page layout
...
#Imputation,EDA and scaling are explained in AQI_Imputation_and_EDA_code.pynb file in github repository
#Loading the imputed dataset
df_air=pd.read_csv('final_imputed_aqi_data.csv')
#fetching unique countries values
countries=list(df_air['Country'].unique())

#Go back to Home Page button in the side bar
...
#Set dropdown options in the side bar
selection = st.sidebar.selectbox("Choose your operation", ["Check Global Statistics", "Check Air Quality for desired region","Learn about Air Quality Index(AQI)","Summary Report by Researcher"])


#Learn about AQI section creation
...
#Check Global Statistics section creation
if selection=='Check Global Statistics':
    ...
    #Bar Graph for countries with best and worst AQI
    with col1:
    ...
    #Map distribution for Air Quality Spread across the globe
    ...
    #Scatterplot for Relationship between pollutants and air quality
...
#Check Air Quality for desired region section
if selection=='Check Air Quality for desired region':
    ...
    if quality[0]=='Good':
        ...
        st.markdown(f'<h1 style="color:#336BFF;font-size:34px;">{"Sorry, Data is not available for this region"}</h1>', unsafe_allow_html=True)
        

#Summary Report by Researcher Section creation
if selection=='Summary Report by Researcher':
    st.markdown("**In our report summary, we present an analysis of the Air Quality Index (AQI) data collected on October 3rd, 2023, from various countries using our Air Sampling equipment. The findings indicate alarming trends in air quality across the globe.**")
    bullet_points = [
    "**Notably, cities in Russia and the USA are experiencing the most severe air pollution, falling under the 'Hazardous' category, while cities in Cyprus and Cameroon boast the best air quality standards.**",
    "**A concerning pattern emerges in populous countries like India and China, where cities with large populations and extensive manufacturing plants exhibit extremely poor AQI levels.**",
    "**Geographically, Asia demonstrates the poorest AQI ratings, contrasting with the Pacific region, which boasts the best AQI ratings.**",
    "**Our visual analysis of AQI categories reveals a direct correlation between Dust Particles and the Air Quality Index, indicating a linear relationship. Conversely, understanding the connection between CO and NO2 pollutants proves challenging when their levels are below 50. However, once these levels surpass 50, they are classified as 'Unhealthy' or 'Hazardous.'**",
    "**Particularly alarming is the prevalence of PM 2.5 and Ozone pollutants, both exhibiting high AQI values and falling within the 'Unhealthy' to 'Hazardous' range. These pollutants pose a significant threat to human health, especially impacting the respiratory system.**",
